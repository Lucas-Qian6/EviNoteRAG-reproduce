"""
Multi-GPU batch evaluation for EviNote-RAG on TriviaQA.

Mirrors `eval_nq_8gpus.py` but loads TriviaQA from Hugging Face. The
retrieval-and-generation loop, the prompt, the scoring, and the multi-GPU
sharding are all identical to the NQ script - only the dataset loader,
output filenames, and labels differ.

Usage:
  1. Start the retriever server in a separate terminal (e.g. dense e5 or BM25).
  2. Run:
       python eval_trivia_8gpus.py \
...
def run_worker(rank, args, data):
    ...
    if not shard or not pending_shard:
        ...
        print(
            f"[GPU {rank}] No pending TriviaQA questions assigned. "
    ...
    print(
        ...
        f"for {len(shard)} TriviaQA questions ({len(pending_shard)} pending)."
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Multi-GPU EviNote-RAG evaluation on TriviaQA"
    )
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="./outputs/eval_trivia_8gpus",
