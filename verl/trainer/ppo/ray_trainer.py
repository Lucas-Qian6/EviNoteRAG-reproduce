...
import re
from RAG.llm_agent.generation import LLMGenerationManager, GenerationConfig
...
class ResourcePoolManager:
    ...
    Mapping
    """
    resource_pool_spec: dict[str, list[int]] # Total available GPUs, e.g. [8, 8]
    mapping: dict[Role, str] # All gpus mapped to [8, 8]
...
class RayPPOTrainer(object):
    ...
    def _validate_eval(self):
        ...
        f1_reward_tensor_lst = []
        retrival_reward_tensor_lst = []
        ...
        if not self.config.do_search:
            for test_data in self.val_dataloader:
                ...
                em_reward_tensor_lst.append(reward_tensor)
            ...
            for batch_dict in self.val_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    f1_reward_tensor = self.val_reward_fn(test_batch, val_type='f1')
                    retrival_reward_tensor = self.val_reward_fn(test_batch, val_type='retrival')

                    em_reward_tensor_lst.append(em_reward_tensor)
                    f1_reward_tensor_lst.append(f1_reward_tensor)
                    retrival_reward_tensor_lst.append(retrival_reward_tensor)
                    data_source_lst.append(test_batch.non_tensor_batch.get('data_source', ['unknown'] * em_reward_tensor.shape[0]))

        em_reward_tensor = torch.cat([rw.sum(-1) for rw in em_reward_tensor_lst], dim=0).cpu()  # (batch_size,)
        ...
        data_source_reward = {}
        for i in range(em_reward_tensor.shape[0]):
    ...
    def fit(self):
        ...
        if self.val_reward_fn is not None and self.config.trainer.get('val_before_train', True):
            val_metrics = self._validate_eval()
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for batch_dict in islice(self.train_dataloader, self.global_steps, None):
                print(f'epoch {epoch}, step {self.global_steps}')
                if batch_dict is None or len(batch_dict) == 0:
                    print("Empty batch size")
                    continue
                metrics = {}
                timing_raw = {}
                ...
                # with _timer('step', timing_raw):
                        ...
                        print("============= Data number (batch_size x rollout.n): ",len(gen_batch),"=============")
                        with _timer('gen', timing_raw):
                            generation_manager.timing_raw = timing_raw
                            final_gen_batch_output = generation_manager.run_llm_loop( # generate answer
                                gen_batch=gen_batch,
                                initial_input_ids=first_input_ids,
                            )
                    ...
                    if self.val_reward_fn is not None and self.config.trainer.test_freq > 0 and \
                        ...
                        with _timer('testing', timing_raw):
                            val_metrics: dict = self._validate_eval()
