"""
Multi-GPU evaluation for EviNote-RAG on TriviaQA with claim-level evidence
organization.

Mirrors `eval_nq_8gpus_claimlevel.py` but loads TriviaQA from Hugging Face.
The retrieval-and-generation loop, the claim-level prompt, the scoring, and
the multi-GPU sharding are all identical to the NQ claim-level script - only
the dataset loader, output filenames, and labels differ.

Usage:
  1. Start the retriever server in a separate terminal (e.g. dense e5 or BM25).
  2. Run:
       python eval_trivia_8gpus_claimlevel.py \
...
def normalize_answers(raw_answers):
    ...
    if isinstance(raw_answers, str):
        ...
        if "aliases" in raw_answers and raw_answers["aliases"]:
            answers = list(raw_answers["aliases"])
            if "value" in raw_answers and raw_answers["value"]:
                answers.append(raw_answers["value"])
        elif "normalized_aliases" in raw_answers and raw_answers["normalized_aliases"]:
            answers = list(raw_answers["normalized_aliases"])
        elif "text" in raw_answers:
            answers = raw_answers["text"]
        elif "value" in raw_answers and raw_answers["value"]:
            answers = [raw_answers["value"]]
        else:
    ...
    for answer in answers:
        ...
        if isinstance(answer, dict):
            answer = answer.get("text", "") or answer.get("value", "")
...
def load_trivia(num_samples=None, dataset_id=None, config="rc.nocontext", split="validation"):
    ...
    if ds is None:
        raise RuntimeError(
            f"Failed to load TriviaQA dataset from {candidates} (config={config}): {last_error}"
        )
    ...
    for idx, item in enumerate(ds):
        ...
        if not answers:
            raise ValueError(f"No valid answers found for TriviaQA example {idx}")
...
def rank_output_path(output_dir, rank):
    return os.path.join(output_dir, f"trivia_results_rank{rank}.jsonl")


def rank_summary_path(output_dir, rank):
    return os.path.join(output_dir, f"trivia_summary_rank{rank}.json")
...
def run_worker(rank, args, data):
    ...
    if not shard or not pending_shard:
        ...
        print(
            f"[GPU {rank}] No pending TriviaQA claim-level questions assigned. "
    ...
    print(
        ...
        f"for {len(shard)} TriviaQA claim-level questions ({len(pending_shard)} pending)."
    )
...
def merge_results(output_dir, model_id, num_gpus, dataset_id, config, split, max_turns, topk):
    merged = {}
    for path in sorted(glob(os.path.join(output_dir, "trivia_results_rank*.jsonl"))):
        with open(path, "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
                if not line:
    ...
    merged_output_file = os.path.join(output_dir, "trivia_results.jsonl")
    ...
    summary_file = os.path.join(output_dir, "trivia_summary.json")
    with open(summary_file, "w", encoding="utf-8") as f:
...
def main():
    parser = argparse.ArgumentParser(
        description="Multi-GPU claim-level EviNote-RAG evaluation on TriviaQA"
    )
    parser.add_argument("--model_id", type=str, required=True, help="HF model ID or local path")
    parser.add_argument("--num_samples", type=int, default=None, help="Number of TriviaQA validation questions")
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="./outputs/eval_trivia_8gpus_claimlevel",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional Hugging Face dataset ID override for TriviaQA",
    )
    parser.add_argument(
        "--config",
        type=str,
        default="rc.nocontext",
        choices=["rc", "rc.nocontext", "unfiltered", "unfiltered.nocontext"],
        help="TriviaQA dataset config (open-domain RAG eval typically uses rc.nocontext)",
    )
    parser.add_argument(
        "--split",
        type=str,
    ...
    print(f"Loading TriviaQA ({args.config}, {label})...")
    data, dataset_id = load_trivia(
