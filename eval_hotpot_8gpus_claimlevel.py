"""
Multi-GPU evaluation for EviNote-RAG on HotpotQA with claim-level evidence
organization.

Mirrors `eval_nq_8gpus_claimlevel.py` but loads HotpotQA from Hugging Face.
...
Usage:
  ...
  2. Run:
       ...
       python eval_hotpot_8gpus_claimlevel.py \
...
def run_worker(rank, args, data):
    ...
    if not shard or not pending_shard:
        ...
        print(
            f"[GPU {rank}] No pending HotpotQA claim-level questions assigned. "
    ...
    print(
        ...
        f"for {len(shard)} HotpotQA claim-level questions ({len(pending_shard)} pending)."
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Multi-GPU claim-level EviNote-RAG evaluation on HotpotQA"
    )
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="./outputs/eval_hotpot_8gpus_claimlevel",
