"""
Multi-GPU batch evaluation for EviNote-RAG on Natural Questions Open.

This keeps the same retrieval-and-generation loop as `eval_single_gpu.py`,
but shards the dataset across multiple GPUs by spawning one worker process per
GPU. Each worker loads a separate model replica on its assigned device.
...
def run_worker(rank, args, data):
    ...
    if not shard or not pending_shard:
        ...
        print(
            f"[GPU {rank}] No pending NQ questions assigned. "
    ...
    print(
        ...
        f"for {len(shard)} NQ questions ({len(pending_shard)} pending)."
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Multi-GPU EviNote-RAG evaluation on Natural Questions Open"
    )
